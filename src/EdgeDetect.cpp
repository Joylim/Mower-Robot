...
void EdgeDetect:: search(int &mm) {
    	...
    	for (int i = 0; i < n; ++ i) {
            	for (int j = 0; j < m; ++ j) {
						...
						if(mat.ptr<uchar>(i)[j] > max_mat)
						{
							max_mat=mat.ptr<uchar>(i)[j] ;
						}
            	}
    	}
		cout<<max_mat<<endl;
...
int EdgeDetect::paint(cv::Mat frame, bool** &result,int &countOfWhite) 
{
		...
		cout<<"the amount of white block is: "<<countOfWhite<<endl;
		return countOfWhite;
}
